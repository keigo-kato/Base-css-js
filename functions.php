...
function set_favicon() {
	$favicon_url = get_template_directory_uri() . "/images/common/favicon.png";
...
function my_wp_enqueue_scripts() {
	...
	$args = array(
		'handle' => 'cardo',
		'src'    => 'https://fonts.googleapis.com/css?family=Cardo:400,700',
		'deps'   => array(),
		'ver'    => false,
		'media'  => 'all'
	);
	...
	$args = array(
		...
		'src'    => $dir_path . '/css/vegas.min.css',
		'deps'   => array(),
		'ver'    => false,
		'media'  => 'all'
	);
	...
	if( is_single() || is_singular() || is_archive() && !is_front_page() ) {
		$args = array(
			'handle' => 'entry-style',
	...
	$args = array(
		'handle' => 'jquery.fatNav.min.css',
		'src'    => $dir_path . '/css/jquery.fatNav.min.css',
		'deps'   => array(),
		'ver'    => false,
		'media'  => 'screen'
	);
	...
	$args = array(
		'handle' => 'vegas.min.js',
		'src'    => $dir_path . '/scripts/vegas.min.js',
		'deps'   => array('jquery'),
		'ver'    => false,
		'in_footer' => true,
	);
	...
	$args = array(
		'handle' => 'jquery.fatNav.min.js',
		'src'    => $dir_path . '/scripts/jquery.fatNav.min.js',
		'deps'   => array('jquery'),
		'ver'    => false,
		'in_footer' => true,
	);
...
// 投稿編集画面で使用するhタグを制限する
add_filter('tiny_mce_before_init', 'my_tine_mce_before_init');
function my_tine_mce_before_init($ar) {
	$ar['block_formats'] = '段落=p;見出し2=h2;見出し3=h3;見出し4=h4;見出し5=h5;';
	return $ar;
}
...
// 投稿編集画面のアイキャッチ画像に注釈をつける
// add_filter('admin_post_thumbnail_html', 'what_is_eyecatch', 10, 2);
// function what_is_eyecatch($content, $post_id){
// 		$post = get_post($post_id);
// 		if ('post' == $post->post_type) {
// 			return $content.'<p class="description">アイキャッチ画像はトップページと投稿詳細ページのサイドバーに表示されます。<br>アイキャッチ画像を登録しない場合は記事本文内の先頭の画像が自動で適用されます。<br>記事内のどこにも画像がない場合はnoimage画像が表示されます。</p>';
// 		}
// }
...
// 投稿のタイトルが指定の文字数以上になったら省略する
function the_short_title($length = 24) {
	$ret = esc_html(get_the_title( $post->ID ));
...
// SPにおいて、画像の親pタグにimg-boxクラスをつける
function img_p_class_content_filter($content) {
	$html = $content;
	$output = preg_replace("/<p.*?><img(.*?)<\/p>/", '<p class="img-box"><img$1</p>', $html);
	return $output;
}
add_filter( 'the_content', 'img_p_class_content_filter' );

// アーカイブページのカテゴリーまたはタグの情報を取得する
